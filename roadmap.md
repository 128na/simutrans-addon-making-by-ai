...
| way（軌道） | 直線・カーブ・分岐... | 単一方向PoC完了（`try-out/way_flat_test/`）、8方向対応は未 |
...
- [x] **平面/薄い形状（道路・線路）のBlenderレンダリングPoC** — `try-out/way_flat_test/`
  - 完全平面(道路)・道床+レール分だけ盛り上がる薄い形状(線路)ともゲーム内表示まで確認
  - 単一方向のみの検証。表示位置・寸法・タイル境界の半透明化・色フリンジ・継ぎ目・
    デフォルトアセットとの相互接続など、この1方向だけでも12周の実機フィードバックが
    必要だった。way系の8方向対応・カーブ/分岐は工数が非常に大きくなる見込みで
    今回は着手せず（車両の方向キャリブレーションと同様、自動化の検討余地あり）
